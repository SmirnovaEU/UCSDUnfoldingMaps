...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseClicked() {
		...
		if (lastClicked != null) {
			...
			Marker selectedMarker = findSelectedMarker();
			if (selectedMarker == null)
				return;
			hideMarkers();

			selectedMarker.setHidden(false);
			lastClicked = (CommonMarker) selectedMarker;
			((CommonMarker) selectedMarker).setClicked(true);
			if (selectedMarker instanceof EarthquakeMarker) {
				double radius = ((EarthquakeMarker) lastClicked).threatCircle();
				List <ScreenPosition> cities = unhideCities(radius, selectedMarker.getLocation());
				((EarthquakeMarker) selectedMarker).setCities(cities);
			} else {
				unhideQuakes(selectedMarker.getLocation());
	...
	// loop over and find selected
	private Marker findSelectedMarker() {
		for (Marker marker : quakeMarkers) {
	...
	// loop over and unhide cities
	private List <ScreenPosition> unhideCities(double r, Location loc) {

		List <ScreenPosition> threadCities = new ArrayList();
		for (Marker marker : cityMarkers) {

			if (marker.getDistanceTo(loc) < r) {
				marker.setHidden(false);
				ScreenPosition sp = ((CommonMarker)marker).getScreenPosition(map);
				threadCities.add(sp);
	...
	// loop over and unhide cities
	private void unhideQuakes(Location cloc) {

		for (Marker marker : quakeMarkers) {

			if (marker.getDistanceTo(cloc) < ((EarthquakeMarker)marker).threatCircle())
				marker.setHidden(false);
		}
	}
